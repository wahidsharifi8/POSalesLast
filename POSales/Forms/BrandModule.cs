    ...
    public partial class BrandModule : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        cn.Open();
                        cm = new SqlCommand("INSERT INTO tbBrand(brand,registeredBy)VALUES(@brand,@registeredBy)", cn);
                        cm.Parameters.AddWithValue("@brand", txtBrand.Text);
                        cm.Parameters.AddWithValue("@registeredBy", int.Parse(user_id));
                        cm.ExecuteNonQuery();
                        cn.Close();
                        MessageBox.Show("Record has been successful saved.", "success");
                        Clear();
                        brand.LoadBrand();
                    }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    cn.Open();
                    cm = new SqlCommand("UPDATE tbBrand SET brand = @brand,registeredBy=@registeredBy WHERE id LIKE'" + lblId.Text + "'", cn);
                    cm.Parameters.AddWithValue("@brand", txtBrand.Text);
                    cm.Parameters.AddWithValue("@registeredBy", int.Parse(user_id));
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Brand has been successfully updated.", "success");
                    Clear();
