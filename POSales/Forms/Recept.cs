    ...
    public partial class Recept : Form
    {
        ...
        public void LoadRecept(string pcash, string pchange)
        {
            ...
            try
            {
                ...
                cn.Open();
                da.SelectCommand = new SqlCommand("SELECT c.id, c.transno, c.pcode, c.price, c.qty, c.disc, c.total, c.sdate, c.status, p.pdesc FROM tbCart AS c INNER JOIN Product AS p ON p.pcode=c.pcode WHERE c.transno LIKE '"+cashier.lblTranNo.Text+"'",cn);
