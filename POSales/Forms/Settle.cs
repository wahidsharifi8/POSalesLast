        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        cn.Open();
                        cm = new SqlCommand("UPDATE Product SET qty = qty - " + int.Parse(cashier.dgvCash.Rows[i].Cells[5].Value.ToString()) + "WHERE pcode= '" + cashier.dgvCash.Rows[i].Cells[2].Value.ToString() + "'", cn);
                        cm.ExecuteNonQuery();
                        cn.Close();
