...
namespace POSales
{
    public partial class Store : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if(txtAddress.Text == "" || txtStName.Text == "")
            {
            ...
            else
            {
                try
                {
                    if (MessageBox.Show("Save store details?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        if (havestoreinfo)
                        {
                            dbcon.ExecuteQuery("UPDATE tbStore SET store = '" + txtStName.Text + "', address= '" + txtAddress.Text + "',registeredBy='"+user_id+"'");
                        }
                        else
                        {
                            dbcon.ExecuteQuery("INSERT INTO tbStore (store,address) VALUES ('" + txtStName.Text + "','" + txtAddress.Text + "','"+user_id+"')");
                        }
