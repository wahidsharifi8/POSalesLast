...
namespace POSales
{
    public partial class Adjustments : Form
    {
        ...
        public void LoadStock()
        {
            ...
            cm = new SqlCommand("SELECT p.pcode, p.barcode, p.pdesc, b.brand, c.category, p.price, p.qty FROM Product AS p INNER JOIN tbBrand AS b ON b.id = p.bid INNER JOIN tbCategory AS c on c.id = p.cid WHERE CONCAT(p.pdesc, b.brand, c.category) LIKE '%" + txtSearch.Text + "%'", cn);
            cn.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    dbcon.ExecuteQuery("UPDATE Product SET qty = (qty - " + int.Parse(txtQty.Text) + ") WHERE pcode LIKE '" + lblPcode.Text + "'");
                }
